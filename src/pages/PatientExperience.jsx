...
const PatientExperience = () => {
    ...
    const docTypes = ['Discharge summary', 'Lab report', 'Prescription'];
        ...
        <div className="patient-experience">
            ...
            <div className="px-input-col">
                ...
                <div className="px-upload-zone">
                    ...
                    <div className="px-upload-title">Upload your health document</div>
                    <div className="px-upload-subtitle">Lab report, prescription, discharge summary — PDF</div>
                </div>

                {/* SAMPLE DOCUMENT TYPE SELECTION */}
                <div>
                    <div className="px-section-label">Choose a sample document type</div>
                ...
                {(isAnalyzing || showResults) && (
                    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: '24px', opacity: isAnalyzing ? 0.6 : 1 }}>
                        <div style={{ textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            {!isAnalyzing && <div style={{ color: '#00C896', fontSize: '12px', fontWeight: 800 }}>VERIFIED SAFE</div>}
                        </div>
                        ...
                        <div style={{ 
                            ...
                            <div style={{ fontSize: '24px', fontWeight: 800, color: 'white', marginBottom: '8px' }}>
                                {isAnalyzing ? '---' : 'Low Clinical Risk'}
                            </div>
                            <div style={{ fontSize: '14px', color: 'var(--text-gray-light)' }}>
                                {isAnalyzing ? 'Auditing model claims...' : 'Every claim in the AI response matches the provided discharge summary and clinical guidelines.'}
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            {[
                                { label: 'Faithfulness', val: isAnalyzing ? 'Scanning...' : '98%' },
                                { label: 'Medical Accuracy', val: isAnalyzing ? 'Analyzing...' : '94%' },
                                { label: 'Source Adherence', val: isAnalyzing ? 'Verifying...' : '100%' }
                            ].map((m, i) => (
                                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    ...
                                    <div style={{ height: '2px', width: '100%', background: 'rgba(255,255,255,0.05)', borderRadius: '2px' }}>
                                        <div style={{ 
                                            ...
                                            background: m.val === '100%' ? '#00C896' : '#4dabf7',
                        ...
                        {!isAnalyzing && (
                            <button className="px-action-btn" style={{ marginTop: 'auto' }} onClick={() => setShowResults(false)}>
